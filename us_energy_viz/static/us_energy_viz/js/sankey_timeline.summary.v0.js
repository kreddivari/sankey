...
const get_totals = function get_totals() {
    ...
    for (let i = 0; i < DATA.length; ++i) {
        ...
        if(DATA[i].milestone !== undefined){ 
            total.milestone = DATA[i].milestone; 
        }
        // else{
        //     total.milestone = "Not found";
        // }
       var totalsum =  total.bio+total.coal+total.gas+total.geo+total.hydro+total.nuclear+total.petro+total.solar+total.wind;;
       yearsum[DATA[i].year] =totalsum;


        totals.push(total);
        flows.push(flow);
        labels.push(label);
        //yearsums.push(yearsum);
    }


    return [totals, flows, labels, show_waste,yearsum];
};
